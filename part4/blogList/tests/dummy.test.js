...
test("レコードが1件のとき、そのレコードを返す", () => {
  const result = listHelper.favoriteBlog(listWithSingleBlog)
  assert.deepStrictEqual(result, listWithSingleBlog[0])
})

test("レコードが複数ある時、いいねが最大のレコードを返す", () => {
  const result = listHelper.favoriteBlog(listWithMultipleBlog)
  assert.deepStrictEqual(result, listWithMultipleBlog[0])
})

test("レコードが0件の場合、nullを返す", () => {
...
describe("Most blogs", () => {
  ...
  test("レコードが1件の時、そのレコードを返す", () => {
    const result = listHelper.mostBlogs(listWithSingleBlog)
    assert.deepStrictEqual(result, { "author": listWithSingleBlog[0].author, "blogs": 1})
  })

  test("レコードがないとき、nullを返す", () => {
